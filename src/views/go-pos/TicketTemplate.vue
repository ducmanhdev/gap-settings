<template>
  <div class="sm:h-full flex flex-col">
    ...
    <a-card class="grow">
      <ul class="custom-tabs-nav">
          ...
          >
            <a-checkbox
              ...
              v-model:checked="formModel[item.activeKey as 'config.pos.owner_receipt_active' | 'config.pos.customer_receipt_active']"
              @change="(event: any) => handleUpdateActiveTicketTemplate(item.activeKey, event.target.checked)"
              @click.stop
            />
              {{item.label}}
          </li>
      ...
      <div
              class="sm:grid grid-cols-[1fr_302px] space-y-7 sm-space-y-0 gap-7"
              :class="{
                'relative after:absolute after:inset-0 after:bg-[rgba(0,0,0,0.4)]':
                (activeKey === 'pos_receipt' && !formModel['config.pos.owner_receipt_active']) ||
                (activeKey === 'pos_receipt_customer' && !formModel['config.pos.customer_receipt_active'])
              }"
      >
        <div>
          <div class="mb-3 sm:min-h-[34px] flex items-center">
            <p class="sub-title mb-0 hidden sm:block">TICKET TEMPLATE</p>
            <p class="main-title mb-0 sm:hidden">Ticket Template</p>
          </div>
        ...
        <div class="flex flex-col">
          <div
            ...
            <p class="sub-title mb-0 hidden sm:block">TICKET PREVIEW</p>
            <p class="main-title mb-0 sm:hidden">Ticket Review</p>
            <button
              type="button"
  ...
  <add-ticket-modal ref="addTicketModalRef" @add-ticket="handleAddTicket"
                    :ticket-list-block-name="ticketListBlockName"></add-ticket-modal>
...
import {useGoPosStore} from "@/stores/go-pos";
...
const tabList = ref([
  {
    label: 'Owner',
    value: 'pos_receipt',
    activeKey: 'config.pos.owner_receipt_active'
  },
  {
    label: 'Customer',
    value: 'pos_receipt_customer',
    activeKey: 'config.pos.customer_receipt_active'
  },
]);
const activeKey = ref(tabList.value[0].value);

const goPosStore = useGoPosStore();
const {
  getTicketInfo,
...
const ticketInfoItems = computed(() => goPosStore.ticketInfo?.items);
const ticketInfoTemplateText = computed(() => goPosStore.ticketInfo?.template_text);
...
watch(activeKey, handleGetTicketInfo, {immediate: true});

const ticketListBlockName = computed(() => goPosStore.ticketListBlockName)
...
const handleOpenAddTicket = () => {
  addTicketModalRef.value?.show(activeKey.value)
};
...
const formModel = ref({
    'config.pos.owner_receipt_active': false,
    'config.pos.customer_receipt_active': false
});
...
const handleGetSettings = async () => {
    try {
        ...
        await Promise.all(
            [
                'config.pos.owner_receipt_active',
                'config.pos.customer_receipt_active'
            ].map((key: any) => {
                return getKeyStorage({key: key})
                    .then(res => (formModel.value as any)[key] = (res as any).value.toString() === '1')
                    .catch(error => error);
            })
        );
    } catch (error) {
...
const isUpdateActiveTicketTemplateLoading = ref(false);
const handleUpdateActiveTicketTemplate = async (key: string, isChecked: boolean) => {
    try {
        isUpdateActiveTicketTemplateLoading.value = true;
        ...
        notification.success({
            message: 'Success',
            description: 'Active Customer Template Success'
        });
    } catch (error) {
        handleError({
            error: error,
        ...
        isUpdateActiveTicketTemplateLoading.value = false;
    }
}
</script>
